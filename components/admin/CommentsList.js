...
import style from "../../styles/admin/CommentsList.module.css";
import api from "../../api";
...
const Main = (props) => {
  ...
  useEffect(() => {
    fetchGamesList(api.adminGetCommentsList, setGamesListData);
  }, []);
  ...
  return (
    <>
      <div className={style.list}>
        {gamesListData.map((comment) => (
          <Comment data={comment} admin={true} authentication={props.authentication} />
        ))}
      </div>
    </>
  );
