...
const Main = (props) => {
    ...
    return (
      <div className={style.main}>
        ...
        <div className={style.content}>
          {isUser2_isUsers_isClubs(props.data) ? <No /> : props.data.children}
        </div>
      </div>
    );
  };
  ...
  const AuthenticatedContent = (props) => {
    let R = <div></div>;
    if (props.data.authentication.state) {
      if (props.data.authentication.cookies.userType == 3) {
        R = <No />;
      } else {
        R = <MainContent data={props.data} />;
      }
    } else {
      R = <No />;
    }
  ...
  return (
    <>
      <AuthenticatedContent data={props} />
