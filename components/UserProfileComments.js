...
const fetcher = (url, props) =>
  fetch(url).then(async (res) => {
    ...
    props.loaded(result.comments.length);
    return result;
  });

const Main = (props) => {
  const { data, error } = useSWR(
    //api.gameComments + `?userId=${props.userId}`,
    api.getUserComments + `?userId=${props.userId}`,
    (url) => fetcher(url, props)
  );
  if (error) return <div>مشکلی در دریافت اطلاعات بوجود آمده.</div>;
